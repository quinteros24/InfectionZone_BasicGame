...
class generadorojo(pygame.sprite.Sprite):
    def __init__(self, pos, lista):
        ...
        self.sonido_ojo = pygame.mixer.Sound(r'D:\UNIVERSIDAD\6to Semestre\Computación Gráfica\Juego1\Juego\sonidos\scream1.ogg' )
    def update(self):
        self.temp -= 1
        self.image = self.lista[self.col]
        if self.col<5:
...
class zombie(pygame.sprite.Sprite):
    ...
    def __init__(self, pos, matriz):
        ...
        self.rect.y = pos[1]
        self.salud = 100
        self.sonido_zombie = pygame.mixer.Sound(r'D:\UNIVERSIDAD\6to Semestre\Computación Gráfica\Juego1\Juego\sonidos\scream2.ogg' )
    def update(self):
        self.image = self.matriz[self.fil][self.col]
        if self.col<2:
            self.col += 1
...
class portal(pygame.sprite.Sprite):
    def __init__(self, pos, lista):
        ...
        self.rect.x = pos[0]
        self.rect.y = pos[1]
        self.sonido_portal = pygame.mixer.Sound(r'D:\UNIVERSIDAD\6to Semestre\Computación Gráfica\Juego1\Juego\sonidos\Rain-loop.ogg' )
    def update(self):
        self.image = self.lista[self.col]
        if self.col<3:
            self.col += 1
...
class sangre(pygame.sprite.Sprite):
    ...
    def __init__(self, pos, lista):
        ...
        self.rect.x = pos[0]
        self.rect.y = pos[1]
        self.sonido_muerte = pygame.mixer.Sound(r'D:\UNIVERSIDAD\6to Semestre\Computación Gráfica\Juego1\Juego\sonidos\scream3.ogg' )
    def update(self):
        self.image = self.lista[self.col]
        if self.col<3:
            self.col += 1
...
class generadoresqueleto(pygame.sprite.Sprite):
    def __init__(self, pos, imagen):
        ...
        self.salud = 100
        self.temp=100
        self.sonido_disparo = pygame.mixer.Sound(r'D:\UNIVERSIDAD\6to Semestre\Computación Gráfica\Juego1\Juego\sonidos\sniper-shot.ogg' )
    def update(self):
        self.temp -= 1
...
class boss(pygame.sprite.Sprite):
    ...
    def __init__(self,pos,matriz):
        ...
        self.vely = 0
        self.salud = 100
        self.temp = random.randrange(40,60)
        self.sonido_boss = pygame.mixer.Sound(r'D:\UNIVERSIDAD\6to Semestre\Computación Gráfica\Juego1\Juego\sonidos\boo_jajaja.ogg' )
        self.laugh = pygame.mixer.Sound(r'D:\UNIVERSIDAD\6to Semestre\Computación Gráfica\Juego1\Juego\sonidos\laugh.ogg' )
        self.start = False
        self.tempattack = 80
    def update(self):
        if self.start and self.tempattack > -10:
            self.tempattack -= 1
        self.temp -= 1
        if self.velx != self.vely:
            self.image = self.matriz[self.fil][self.col]
...
class explosion(pygame.sprite.Sprite):
    ...
    def __init__(self, pos, lista):
        ...
        self.rect.x = pos[0]
        self.rect.y = pos[1]
        self.sonido_explosion = pygame.mixer.Sound(r'D:\UNIVERSIDAD\6to Semestre\Computación Gráfica\Juego1\Juego\sonidos\Small-explosion.ogg' )
        self.temp = 80
    def update(self):
        self.temp -= 1
        self.image = self.lista[self.col]
...
if __name__ == '__main__':
    ...
    Fuente2 = pygame.font.Font(r'D:\UNIVERSIDAD\6to Semestre\Computación Gráfica\Juego1\Juego\upheavtt.ttf', 27) #Fuente del mensaje que arroja 
    Fuente3 = pygame.font.Font(None, 37)
...
#------------------------------------------------------------------------------------------------------------------RECORTE PERSONAJE
    ...
    fondo_bienv = pygame.image.load(r'D:\UNIVERSIDAD\6to Semestre\Computación Gráfica\Juego1\Juego\fondo_bienv.png')
    ...
    #------------------------------------------------------------------------------------------------------------------RECORTE BOSS
    im_boss = pygame.image.load(r'D:\UNIVERSIDAD\6to Semestre\Computación Gráfica\Juego1\Juego\final.png')
    info_img = im_boss.get_rect()
    ...
    for i in range(sp_alto):
        aux = []
        for j in range(sp_ancho):
            cuadro = im_boss.subsurface(desplazamiento_x*j,desplazamiento_y*i,desplazamiento_x,desplazamiento_y)
            aux.append(cuadro)
        matrizboss.append(aux)

    im_ex = pygame.image.load(r'D:\UNIVERSIDAD\6to Semestre\Computación Gráfica\Juego1\Juego\explo.png')
...
#------------------------------------------------------------------------------------------------------------------INICIALIZACION
    ...
    mod = modificador([30*32,50*32],matrizmod, 0)
    ...
    posboss1 = [28*32,65*32]
    bossgroup = pygame.sprite.Group()
    jefe = boss(posboss1, matrizboss)
    bossgroup.add(jefe)
    ...
    sonido_explosion = True

    cambiotema = True

    #Limites del Boss
    lim_b_top = 59*32 + 100
    lim_b_bottom = alto_fondo - 200
    lim_b_left = 32 + 100
    lim_b_right = 60*32 - 100

    salgo = False
    pygame.mixer.init()
    pygame.mixer.music.load(r'D:\UNIVERSIDAD\6to Semestre\Computación Gráfica\Juego1\Juego\sonidos\soundtrack.ogg' )
...
#------------------------------------------------------------------------------------------------------------------PANTALLA DE BIENVENIDA
    ...
    while not fin:
        for event in pygame.event.get():
                ...
                pantalla.fill(NEGRO)
                pantalla.blit(fondo_bienv, [0,0])
                ubicacion_titulo = [250, 20]
                texto_titulo = "|-|-|-|-|-|-| INFECTION ZONE |-|-|-|-|-|-|"

                ubicacion_mensaje = [250,200]
                texto = "1. PARA INICIAR EL JUEGO OPRIME 'SPACE'"

                ubicacion_mensaje2 = [20,400]
                texto2 = "1. Pocion Roja: Repone parte de la salud del jugador"

                ubicacion_mensaje3 = [20,430]
                texto3 = "2. Pocion Amarilla: Otorga una mejora en arma con mas daño hacia tus enemigos"
                
                ubicacion_mensaje4 = [20,460]
                texto4 = "3. Pocion Morada: Permite obtener tu arma inicial y ayudarte a sobrevivir"

                ubicacion_mensaje5 = [20,490]
                texto5 = "4. Pocion Azul: Otorga resistencia al daño para el jugador evitando una muerte rapida"
                # texto2 = """Al entrar en esta ciudad abandonada, tendras que cruzar un camino lleno de diversos enemigos
                # quienes te atacaran y te eliminaran si das el paso equivocado, recuerda prestar atencion a todo tu entorno
                # ya que podras encontrar pociones para ayudarte en tu travesia:
                # 1. Pocion Roja: Repone parte de la salud del jugador
                # 2. Pocion Amarilla: Otorga una mejora en arma con mas dañp hacia tus enemigos
                # 3. Pocion Morada: Permite obtener tu arma inicial y ayudarte a sobrevivir
                # 4. Pocion Azul: Otorga resistencia al daño para el jugador evitando una muerte rapida
                
                # Tendras que abrirte camino entre las 4 salas, busca pociones y elimina a todos tus objetivos hasta llegar 
                # a la ultima estacion, derrota el jefe final y logra escapar de esta ciudad, recuerda que estas en INFECTION ZONE
                # y tu destino depende unicamente de ti, disfruta y diviertete en esta nueva aventura"""
                ubicacion_imagen = [600,530]
                for i in matrizmod:
                    pantalla.blit(i, ubicacion_imagen)
                    ubicacion_imagen[0] += 40

                ubicacion_autores1 = [50,600]
                texto_autores1 = "|*|*|*|*|*|*| SANTIAGO QUINTERO ANGARITA |*|*|*|*|*|*|"

                ubicacion_autores2 = [100,630]
...
#------------------------------------------------------------------------------------------------------------------SONIDOS
    #pygame.mixer.Sound('D:/UNIVERSIDAD/6to Semestre/Computación Gráfica/Segunda Parte/practica/Jugador/scream.ogg' )
    #D:\UNIVERSIDAD\6to Semestre\Computación Gráfica\Juego1\Juego\sonidos\boo_jajaja.ogg
...
#------------------------------------------------------------------------------------------------------------------SUCESOS
        ...
        if stage == 4 and cambiotema:
            cambiotema = False
            pygame.mixer.music.stop()
            pygame.mixer.init()
            pygame.mixer.music.load(r'D:\UNIVERSIDAD\6to Semestre\Computación Gráfica\Juego1\Juego\sonidos\bosstheme.ogg' )
            pygame.mixer.music.set_volume(0.2)
            pygame.mixer.music.play()
...
#------------------------------------------------------------------------------------------------------------------CONTROL MODIFICADORES
        ...
        if len(lista_colision) > 0:
            for m in lista_colision:
                ...
                if m.col == 0 and j1.salud < 100 and stage == 4:
...
#------------------------------------------------------------------------------------------------------------------CONTROL PORTALES
        ...
        if entra_por1:
            ...
            if f_posx < (-1*(60*32)):
                ...
                entra_por1 = False
            #if sonido_portal1:
            port1.sonido_portal.play()
            #    sonido_portal1 = False
...
#------------------------------------------------------------------------------------------------------------------CONTROL GEN OJOS
        if len(generadores_ojos) == 0 and auxiliar_portal2 == 1:
            crear_portal2 = True
...
#------------------------------------------------------------------------------------------------------------------CONTROL BOSS MOVIMIENTO
        if stage == 4 and startBOSS:
            startBOSS = False
            for j in bossgroup:
                j.velx = 20
        

        #print([jefe.rect.x,jefe.rect.y],' limder:',lim_b_right,'limabaj:',lim_b_bottom,' limiz:',lim_b_left,' limarri:',lim_b_top)
        for j in bossgroup:
            if stage == 4 and oleadasboss > 0:
                #Cuando el jefe choca con sus limites
                if j.rect.right > lim_b_right and j.velx > 0:
                    j.vely = 0
                    j.velx = -1*j.velx
                    j.fil = 3
                elif j.temp < 0 and j.velx > 0:
                    #Cuando va a la derecha voltea hacia abajo
                    j.fil = 0
                    j.vely = j.velx
                    j.velx = 0
                    j.temp = random.randrange(20,150)
                if j.rect.left < lim_b_left and j.velx < 0:
                    j.vely = 0
                    j.velx = -1*j.velx
                    j.fil = 2
                elif j.temp < 0 and j.velx < 0:
                    #Cuando va hacia la izquierda voltea hacia arriba
                    j.fil = 1
                    j.vely = j.velx
                    j.velx = 0
                    j.temp = random.randrange(20,150)
                if j.rect.top < lim_b_top and j.vely < 0:
                    j.velx = 0
                    j.vely = -1*j.vely
                    j.fil = 0
                elif j.temp < 0 and j.vely < 0:
                    #Cuando va hacia arriba voltea hacia la derecha
                    j.fil = 2
                    j.velx = -1*j.vely
                    j.vely = 0
                    j.temp = random.randrange(20,150)
                if j.rect.bottom > lim_b_bottom and j.vely > 0:
                    j.velx = 0
                    j.vely = -1*j.vely
                    j.fil = 1
                elif j.temp < 0 and j.vely > 0:
                    #Cuando va hacia abajo voltea hacia la izquierda
                    j.fil = 3
                    j.velx = -1*j.vely
                    j.vely = 0
                    j.temp = random.randrange(20,150)
            ...
            for b in bossgroup:
                ...
                if b.tempattack < 0:
                    ...
                    if len(ls_col) > 0:
                        ...
                        b.tempattack = 40
                    if mod_arma == 2:
                        ls_col = pygame.sprite.spritecollide(b,balas, False)
                        if len(ls_col) > 0:
                            b.salud -= 2 #50 golpes para morir

        for b in bossgroup:
            if b.salud < 1:
                posiboss = [b.rect.x,b.rect.y]
                if sonido_explosion:
                    e = explosion(posiboss, listaex)
                    explosiones.add(e)
                    e.sonido_explosion.play()
                    if oleadasboss > 1:
                        jefe2 = boss(posiboss, matrizboss)
                        bossgroup.add(jefe2)
                        b.sonido_boss.play()
                        startBOSS = True
                    oleadasboss -= 1
...
#------------------------------------------------------------------------------------------------------------------CONTROL VIDA JUGADOR
        if j1.salud < 1:
            ...
            j1.kill()
            if sonido_fin:
                s1.sonido_muerte.play()
                #s1.sonido_muerte.stop()
                sonido_fin = False
            mostrar_sangre = True
...
#------------------------------------------------------------------------------------------------------------------MENSAJES
        ...
        if stage == 1:
            if mod_arma == 0:
                info = 'Tip: pocion morada - Pulsa ESPACIO para disparar'
                colormensaje = NEGRO
                txt_info = Fuente3.render(info, True, colormensaje)
                pantalla.blit(txt_info, [400,600])

        #Stage 2
        if stage == 2:
            if mod_arma == 1:
                info = 'Tip: el arma actual no daña los fantasmas ni los ojos'
                colormensaje = NEGRO
                txt_info = Fuente3.render(info, True, colormensaje)
                pantalla.blit(txt_info, [300,600])

        #Stage 4
        if stage == 4:
            for j in bossgroup:
                info = "VIDA ENEMIGO: "+str(j.salud)
                color_jefe = ROJO
                txt_info = Fuente2.render(info, True, color_jefe)
                pantalla.blit(txt_info, [350,70])
            if j1.salud <= 25:
                info = 'ATENCION! Salud demasiado baja'
                colormensaje = ROJO
                txt_info = Fuente3.render(info, True, colormensaje)
                pantalla.blit(txt_info, [400,600])
